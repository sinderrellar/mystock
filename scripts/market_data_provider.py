...
class MarketDataProvider:
    ...
    def get_bars(self, code: str, market: str, days: int = 120,
        ...
        if self.mongo_available:
            ...
            if quotes:
                ...
                if as_of_date or not result["stale"]:
                    return result
    ...
    def get_trend_signal(
        ...
        if as_of_date is None and latest_bar_date != today_str:
            try:
    ...
    def _get_margin_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            today = _utc_now().strftime("%Y%m%d")
            start = (_utc_now() - timedelta(days=15)).strftime("%Y%m%d")
            df = pro.margin(start_date=start, end_date=today)
            if df is None or df.empty:
                return {"available": False}

            daily = []
            for trade_date, rows in df.groupby("trade_date"):
                daily.append({
                    "trade_date": str(trade_date),
                    "rzye": sum(_to_float(v, 0) or 0 for v in rows.get("rzye", [])),
                    "rqye": sum(_to_float(v, 0) or 0 for v in rows.get("rqye", [])),
                    "rzmre": sum(_to_float(v, 0) or 0 for v in rows.get("rzmre", [])),
                })
            daily.sort(key=lambda row: row["trade_date"], reverse=True)
            latest = daily[0]
            previous = daily[1] if len(daily) > 1 else None
            rzye_now = latest["rzye"]
            rzye_prev = previous["rzye"] if previous else 0
            return {
                "available": True, "source": "tushare",
                "rzye": round(rzye_now / 1e8, 2),
                "rqye": round(latest["rqye"] / 1e8, 2),
                "rzmre": round(latest["rzmre"] / 1e8, 2),
                "rzye_change": (round((rzye_now - rzye_prev) / rzye_prev * 100, 3)
                                if rzye_prev > 0 else None),
